...
export class dbConnection {
  ...
  insert: (data: any, tableName: string) => Promise<any[]> = async (
    data,
    tableName
  ) => {
    const next_id = data.id
      ? data.id
      : (await this.dbInstance(tableName).max("id"))[0].max + 1;
  ...
  getArtistTypes: (artistID: number) => dbQueryFunc = (artistID) => () =>
    this.dbInstance
      .distinct("t.id", "t.name")
      .select()
      .from("types as t")
      .leftJoin("entries as e", "e.type", "t.id")
  ...
  getReleases: (entry: number) => dbQueryFunc = (entry) => () =>
    this.dbInstance.select().from("releases").where({
      entry_id: entry,
    });

  getRelease: (id: Number) => dbQueryFunc = (id) => () =>
    this.dbInstance.table("releases").first().where({
      id,
    });

  getEntry: (id: Number) => dbQueryFunc = (id) => () =>
    this.dbInstance.table("entries").first().where({
      id,
    });

  getLabels: () => dbQueryFunc = () => () => this.dbInstance("labels").select();
  ...
  addRelease: (release: any) => () => Promise<{ release_id: Number }> =
    (release) => () =>
      this.insert(release, "releases").then(([release_id]) => ({ release_id }));

  updateRelease: (release: any) => dbQueryFunc = (release) => () =>
    this.dbInstance("releases")
      .where({ id: release.id })
      .update(release)
      .then(this.getRelease(release.id));

  getReleaseTracks: (release_id: any) => dbQueryFunc = (release_id) => () =>
    this.dbInstance("compositions as c")
      .join("tracks as t", "c.id", "t.composition_id")
      .join("release_tracks as rt", "rt.track_id", "t.id")
      .select([
        "name",
        "t.id",
        "composition_id",
